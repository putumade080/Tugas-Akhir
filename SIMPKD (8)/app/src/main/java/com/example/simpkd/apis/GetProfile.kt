...
fun getProfile(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.GET,
            "${Constants.BASE_URL}/profile/", headers, requestParams,
            fileRequestParams, {
                val dataString = getResponseData(
                    it, contextView, requestParams,
                    fileRequestParams, callback, errorCallback, ::getProfile, showMessage
                )

                if (dataString != null) {
                    val data = Gson().fromJson(dataString, Pegawai::class.java)
                    callback(data)
                }
            }, { errorCallback() }).start()
        return
