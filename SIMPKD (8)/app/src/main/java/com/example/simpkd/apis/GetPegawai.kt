...
fun getPegawai(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.GET,
            "${Constants.BASE_URL}/pegawai/", headers, requestParams,
            fileRequestParams, {
                val dataString = getResponseData(
                    it, contextView, requestParams,
                    fileRequestParams, callback, errorCallback, ::getPegawai, showMessage
                )

                if (dataString != null) {
                    val data = Gson().fromJson(dataString, Array<Pegawai>::class.java)
                    callback(data)
                }
            }, { errorCallback() }).start()
        return
