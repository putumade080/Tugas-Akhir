...
fun getSuratPerintahTugas(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.GET,
            "${Constants.BASE_URL}/surat-perintah-tugas/", headers, requestParams,
            fileRequestParams, {
                ...
                if (dataString != null) {
                    val data = Gson().fromJson(dataString, Array<SuratPerintahTugas>::class.java)
                    callback(data)
                }
            }, { errorCallback() }).start()
        return
