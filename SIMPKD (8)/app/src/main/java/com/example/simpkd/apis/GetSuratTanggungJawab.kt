...
fun getSuratTanggungJawab(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.GET,
            "${Constants.BASE_URL}/surat-tanggung-jawab/", headers, requestParams,
            fileRequestParams, {
                val dataString = getResponseData(
                    it, contextView, requestParams,
                    fileRequestParams, callback, errorCallback, ::getSuratTanggungJawab, showMessage
                )

                if (dataString != null) {
                    val data = Gson().fromJson(dataString, Array<SuratTanggungJawab>::class.java)
                    callback(data)
                }
            }, { errorCallback() }).start()
        return
