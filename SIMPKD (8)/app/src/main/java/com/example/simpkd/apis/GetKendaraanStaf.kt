...
fun getKendaraanStaf(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.GET,
            "${Constants.BASE_URL}/kendaraan/get-staff-vehicle/", headers, requestParams,
            fileRequestParams, {
                val dataString = getResponseData(
                    it, contextView, requestParams,
                    fileRequestParams, callback, errorCallback, ::getKendaraanStaf, showMessage
                )

                if (dataString != null) {
                    val data = Gson().fromJson(dataString, Array<Kendaraan>::class.java)
                    callback(data)
                }
            }, { errorCallback() }).start()
        return
