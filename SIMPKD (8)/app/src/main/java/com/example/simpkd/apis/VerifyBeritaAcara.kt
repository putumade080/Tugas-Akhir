...
fun verifyBeritaAcara(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.POST,
            "${Constants.BASE_URL}/berita-acara/verify/", headers, requestParams,
            fileRequestParams, {
                val dataString = getResponseData(
                    it, contextView, requestParams,
                    fileRequestParams, callback, errorCallback, ::verifyBeritaAcara, showMessage
                )
