...
fun getBeritaAcara(contextView: View, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(contextView.context, Request.Method.GET,
            "${Constants.BASE_URL}/berita-acara/", headers, requestParams,
            fileRequestParams, {
                val dataString = getResponseData(
                    it, contextView, requestParams,
                    fileRequestParams, callback, errorCallback, ::getBeritaAcara, showMessage
                )

                if (dataString != null) {
                    val data = Gson().fromJson(dataString, Array<BeritaAcara>::class.java)
                    callback(data)
                }
            }, { errorCallback() }).start()
        return
